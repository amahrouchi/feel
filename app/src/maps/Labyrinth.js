    ...
    _generateTilemapJSON(size) {
        ...
        for (let line of this._matrix) {
            ...
            for (let val of line) {
                ...
                let topLeftIsPath   = typeof(this._matrix[y - 1]) !== 'undefined' && typeof(this._matrix[y - 1][x - 1]) !== 'undefined' && this._matrix[y - 1][x - 1] === 1;
                let topRightIsPath  = typeof(this._matrix[y - 1]) !== 'undefined' && typeof(this._matrix[y - 1][x + 1]) !== 'undefined' && this._matrix[y - 1][x + 1] === 1;
                let downLeftIsPath  = typeof(this._matrix[y + 1]) !== 'undefined' && typeof(this._matrix[y + 1][x - 1]) !== 'undefined' && this._matrix[y + 1][x - 1] === 1;
                let downRightIsPath = typeof(this._matrix[y + 1]) !== 'undefined' && typeof(this._matrix[y + 1][x + 1]) !== 'undefined' && this._matrix[y + 1][x + 1] === 1;
                ...
                if (val === 0) {
                    ...
                    for (let j = 0; j < LabyrinthConfig.MAP_SIZE_RATIO; j++) {
                        for (let k = 0; k < LabyrinthConfig.MAP_SIZE_RATIO; k++) {
                            ...
                            // Diagonals
                            if (downRightIsPath && !downIsPath && !rightIsPath) {
                                if (k === lastIndex && j === lastIndex) {
                                    wallMatrix[yTilePosition][xTilePosition] = LabyrinthConfig.WALL_TILES.bosquet.up_left;
                                }
                            }

                            if (downLeftIsPath && !downIsPath && !leftIsPath) {
                                if (k === lastIndex && j === 0) {
                                    wallMatrix[yTilePosition][xTilePosition] = LabyrinthConfig.WALL_TILES.bosquet.up_right;
                                }
                            }

                            if (topRightIsPath && !topIsPath && !rightIsPath) {
                                if (k === 0 && j === lastIndex) {
                                    wallMatrix[yTilePosition][xTilePosition] = LabyrinthConfig.WALL_TILES.bosquet.down_left;
                                }
                            }

                            if (topLeftIsPath && !topIsPath && !leftIsPath) {
                                if (k === 0 && j === 0) {
                                    wallMatrix[yTilePosition][xTilePosition] = LabyrinthConfig.WALL_TILES.bosquet.down_right;
                                }
                            }
